...
"""
    ...
    return template.replace("{escaped_script}", escaped).replace("{webhook_url}", webhook_url)
...
async def deobf_command(ctx, *, loadstring):
    ...
    # Formato 4: game:HttptGet("URL") (con dos t)
    if not url_match:
        url_match = re.search(r"game:HttptGet\(['\"]([^'\"]+)['\"]\)", cleaned_text)
    
    # Formato 5: URL directa
    if not url_match:
...
# =============================================
# ... (todos los slash commands que tenías antes, sin cambios)
# Para no alargar este mensaje, los incluyo en el archivo completo.
